...
import com.bosonshiggs.imagecompressor.helpers.PathUtils;
...
public class ImageCompressor extends AndroidNonvisibleComponent implements ActivityResultListener {
    ...
    @SimpleProperty(
        description = "Enable or disable logging to Logcat"
    )
    ...
    public String PathFromUri(String stringUri) {
        if (flagLog) Log.d("ImageCompressor", "Converting URI to path: " + stringUri);
        ...
        try (Cursor cursor = container.$context().getContentResolver().query(uri, projection, null, null, null)) {
            ...
            if (flagLog) Log.e("ImageCompressor", "Error converting URI to path", e);
        }
        return stringUri;
    }
    ...
    public void CompressImage(@Asset final String imagePath, final String imageName, final int quality, final int id) {
        if (flagLog) Log.d("ImageCompressor", "Starting image compression: " + imagePath);
            ...
            public void run() {
                try {
                    ...
                    if (imageType.equals("jpg") || imageType.equals("jpeg")) {
                        ...
                        if (flagLog) Log.e("ImageCompressor", "Unsupported file format: " + imageType);
                        fos.close();
                        return;
                    }
                    ...
                    container.$form().runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (flagLog) Log.i("ImageCompressor", "Image compressed and saved: " + imageFile.getAbsolutePath());
                        }
                    });
                } catch (final Exception e) {
                    if (flagLog) Log.e("ImageCompressor", "Error compressing and saving image", e);
                    container.$form().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
